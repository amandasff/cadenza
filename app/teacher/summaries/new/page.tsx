      ...
      <div className="r-two-col" style={{ gridTemplateColumns:"1fr 300px" }}>
        <div style={{ display:"flex", flexDirection:"column", gap:"1rem" }}>
          <div style={{ background:"var(--white)", borderRadius:20, padding:"1.5rem", border:"1.5px solid var(--border)" }}>
            ...
            <button onClick={()=>setShowSuggested(true)} style={{ marginTop:"1rem", background:"var(--sky-bg)", color:"var(--sky)", border:"1.5px solid var(--sky-light)", borderRadius:100, padding:"0.65rem 1.25rem", cursor:"pointer", fontFamily:"Nunito,sans-serif", fontWeight:700, fontSize:"0.875rem", display:"inline-flex", alignItems:"center", gap:"0.375rem" }}><Sparkles size={14} strokeWidth={1.5} /> Suggest Path Nodes</button>
          </div>          {showSuggested && (
            <div style={{ background:"var(--white)", borderRadius:20, padding:"1.5rem", border:"1.5px solid var(--border)" }}>
              ...
              <div style={{ display:"flex", flexDirection:"column", gap:"0.75rem" }}>
                {suggested.map((g,i) => (
                  <div key={g.id} onClick={()=>setSelected(s=>s.includes(i)?s.filter(x=>x!==i):[...s,i])} style={{ display:"flex", alignItems:"flex-start", gap:"0.75rem", padding:"0.875rem 1rem", border:`1.5px solid ${selected.includes(i)?g.color:"var(--border)"}`, borderRadius:16, cursor:"pointer", background:selected.includes(i)?g.bg:"var(--cream)", transition:"all 0.15s" }}>
                    <div style={{ width:20, height:20, borderRadius:6, border:`2px solid ${g.color}`, background:selected.includes(i)?g.color:"transparent", display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0, marginTop:1 }}>{selected.includes(i)&&<Check size={12} strokeWidth={2.5} color="white" />}</div>
                    <div style={{ flex:1 }}>
                      ...
                      <div style={{ display:"flex", gap:"0.5rem", marginTop:4 }}>
                        ...
                        <span style={{ color:"var(--butter)", fontSize:"0.72rem", fontFamily:"Nunito,sans-serif", fontWeight:700, display:"inline-flex", alignItems:"center", gap:"0.2rem" }}><Star size={10} fill="currentColor" strokeWidth={0} /> {g.points} pts</span>
