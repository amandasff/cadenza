...
export default function PublicProfileClient({ username, data }: Props) {
  ...
  const { profile, tracks, collectibles, featuredComposer, themeSong } = data;
  ...
  return (
    <div style={{ minHeight: "100dvh", background: "#F8F6F2", fontFamily: "Inter, sans-serif" }}>
      ...
      <div style={{ maxWidth: 560, margin: "0 auto", padding: "2rem 1.25rem 4rem" }}>
        ...
        {/* ── AI Persona / Studio Bio ── */}
        {(profile.studio_persona || profile.studio_bio) && (
          <div style={{ background: "#FDFCFA", border: "1px solid #E8E3D9", borderRadius: 12, padding: "1.125rem 1.25rem", marginBottom: "1.5rem" }}>
            {profile.studio_persona && (
              <div style={{ fontSize: "0.6875rem", fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", color: "#9A9590", marginBottom: "0.375rem" }}>
                {profile.studio_persona}
              </div>
            )}
            {profile.studio_bio && (
              <div style={{ fontFamily: "Cormorant Garamond, Georgia, serif", fontSize: "1.0625rem", color: "#2C2824", lineHeight: 1.6, fontStyle: "italic" }}>
                {profile.studio_bio}
              </div>
            )}
          </div>
        ...
        {featuredComposer && (
          <div style={{ marginBottom: "1.5rem" }}>
            ...
            <div style={{ background: "#FDFCFA", border: `1px solid ${(RARITY_COLOR[featuredComposer.rarity] ?? "#9A9590") + "55"}`, borderRadius: 12, padding: "1rem 1.125rem", display: "flex", alignItems: "center", gap: "1rem" }}>
              {featuredComposer.image_path ? (
                <img src={featuredComposer.image_path} alt={featuredComposer.composer_name}
                  style={{ width: 56, height: 56, borderRadius: "50%", objectFit: "cover", border: `2px solid ${RARITY_COLOR[featuredComposer.rarity] ?? "#9A9590"}`, flexShrink: 0 }} />
              ) : (
                <div style={{ width: 56, height: 56, borderRadius: "50%", background: (RARITY_COLOR[featuredComposer.rarity] ?? "#9A9590") + "22", border: `2px solid ${RARITY_COLOR[featuredComposer.rarity] ?? "#9A9590"}`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: "1.5rem", flexShrink: 0 }}>🎼</div>
              )}
              <div>
                <div style={{ fontWeight: 600, fontSize: "0.9375rem", color: "#2C2824" }}>{featuredComposer.composer_name}</div>
                <div style={{ fontSize: "0.75rem", color: "#9A9590", marginTop: "0.125rem" }}>{featuredComposer.era}</div>
                <div style={{ fontSize: "0.625rem", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.06em", color: RARITY_COLOR[featuredComposer.rarity] ?? "#9A9590", marginTop: "0.25rem" }}>
                  {featuredComposer.rarity}
                </div>
              </div>
            </div>
          </div>
        ...
        {validCollectibles.length > 0 && (
          <div style={{ marginBottom: "2rem" }}>
            <div style={{ fontSize: "0.625rem", fontWeight: 600, letterSpacing: "0.1em", textTransform: "uppercase", color: "#9A9590", marginBottom: "0.75rem" }}>
              Composer Collection · {validCollectibles.length}
            </div>
